...
namespace Webcam_Test
{
    ...
    public class Form1 : System.Windows.Forms.Form
    {
        ...
        private void WebCamCapture_ImageCaptured(object source, WebCam_Capture.WebcamEventArgs e)
        {
            ...
            if (File.Exists("c:\\stream.bmp"))
            {
                File.Delete("c:\\stream.bmp");
            }
            // todo: send to server

            byte[] bytes = ImageConverter.imageToByteArray(e.WebCamImage);
            Image img = ImageConverter.byteArrayToImage(bytes);
            img.Save("c:\\stream.bmp");
            
            _clientControl.SendWebcamCapture(bytes);
        }
